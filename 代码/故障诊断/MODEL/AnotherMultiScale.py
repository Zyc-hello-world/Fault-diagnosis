...
from six import u
import keras
from MODEL.MyNet import CBAM, CAM
...
def MultiScale(features,channels, faults):
    ...
    b1 = block(input_tensor, 32, 7, strides=1)
    b2 = block(b1, 64, 5, strides=1)
    b3 = block(b2, 128, 3, strides=1)
    out = multi_conv(input_tensor,16,16,32,8,16,16)
    out = layers.MaxPooling1D(2, padding="same")(out)
    out = multi_conv(out,32,32,64,16,32,32)
    out = layers.MaxPooling1D(2, padding="same")(out)
    out = multi_conv(out,32,48,96,24,64,64)
    out = layers.MaxPooling1D(2, padding="same")(out)
    out = layers.concatenate([b3, out])
