...
# https://codechina.csdn.net/mirrors/myme5261314/dbn_tf/-/blob/master/rbm_tf.py
"""
...
class RBM(object):
    ...
    def train(self, X):
        ...
        _vw = tf.placeholder(tf.float32, [self._input_size, self._output_size])
        _vhb = tf.placeholder(tf.float32, [self._output_size])
        _vvb = tf.placeholder(tf.float32, [self._input_size])
        ...
        with tf.Session() as sess:
            ...
            old_hb = self.init_hb
            old_vb = self.init_vb
            for i in range(self._opts._pre_train_epoches):
    ...
    def rbmup(self, X):
        ...
        """
        input_X = tf.constant(X, tf.float32)
        _w = tf.constant(self.w, tf.float32)
        _hb = tf.constant(self.hb, tf.float32)
